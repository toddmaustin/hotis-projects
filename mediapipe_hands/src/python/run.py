import os
import threading, json, cv2, sys, pickle, math
import numpy as np
...
class MediapipeHandsDemo:
    ...
    def drawLandmarks(self, frame, data, is_left):
        ...
        for myHand in allhands:
            ...
            vx = myHand[8][0] - myHand[0][0];
            vy = -(myHand[8][1] - myHand[0][1]);
            if is_left:
                print("#lh# (", myHand[0][0], ",", 500-myHand[0][1],"),", int(math.degrees(math.atan2(vy,vx))), ",", int(math.sqrt(vx**2 + vy**2)))
            else:
                print("#rh# (", myHand[0][0], ",", 500-myHand[0][1],"),", int(math.degrees(math.atan2(vy,vx))), ",", int(math.sqrt(vx**2 + vy**2)))
        return frame
